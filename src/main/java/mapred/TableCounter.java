...
public class TableCounter {
    ...
    /**
     * Returns a map of <TableName, Record Count> for every table. I assume that it reads the table's internal metadata instead of counting every
     * single row since it happens relatively quickly.
     * 
    ...
    public static Map<String, Long> getTableCount() throws ThriftSecurityException, TException {
        ...
        log.info("\nFetching data now...\n");
        MasterClientService.Iface client = MasterClient.getConnection((ClientContext) HdfsZooInstance.getInstance());
